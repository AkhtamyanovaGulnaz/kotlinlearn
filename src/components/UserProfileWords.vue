<template>
  <div>
    <v-card v-if="currentWord" class="mt-2" dark>
      <v-card-title>
        <div class="headline">
          <original-word :word="currentWord" :size="45" :showAudio="true" canPronounceWord></original-word>
        </div>
      </v-card-title>
      <v-divider> </v-divider>
      ...
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn dark small color="primary" @click="currentWord.showTrans = true" v-if="!currentWord.showTrans"> <v-icon>visibility</v-icon> Показать перевод </v-btn>
        <v-btn small dark color="success" @click="processWord(currentWord)"> <v-icon>check</v-icon> Я запомнил(а) это слово </v-btn>
      </v-card-actions>
    </v-card>
    <v-card>
      ...
      <v-card-title v-else>
        <span>Нет слов для изучения. Добавляйте новые слова из курсов</span>
      </v-card-title>
      <v-card-text>
        <v-list>
  ...
  methods: {
    setWords() {
      ...
      for (let property in userWords) {
        ...
        let isAvailable = word.nextShowDate instanceof Date ? word.nextShowDate <= new Date() : word.nextShowDate.toDate() <= new Date()
